...
model, schema, cryptographic integrity, signature, trust, and verification
requirements implemented by `src/mcc_attestation/` in PR-1. It has not yet
undergone the Final Acceptance Review process used elsewhere in this
repository's Specification Program (see
...
schema and required content, its cryptographic integrity and signature
requirements, its Attester trust model, its deterministic verification
procedure, and its explicit non-authority boundary.
...
| `claims` | REQUIRED | object | Structured, deterministic (canonically serializable) assessment data; semantics are interpreted by policy/Control, never by a conforming verifier |
| `action_hash` | REQUIRED | string, non-empty | Binds the attestation to a specific proposed action |
| `payload_hash` | OPTIONAL | string, non-empty | Binds the attestation to a specific payload |
| `scope` | REQUIRED | string, non-empty | The scope this attestation applies to |
| `policy_hash` | OPTIONAL | string, non-empty | Deterministic policy binding (content-hash form) |
| `policy_version` | OPTIONAL | string, non-empty | Deterministic policy binding (version-label form) |
| `provenance` | REQUIRED | object | Free-form structured provenance (e.g. model identity, input reference) |
| `issued_at` | REQUIRED | integer | Unix seconds |
| `not_before` | REQUIRED | integer | Unix seconds; validity window start |
| `expires_at` | REQUIRED | integer | Unix seconds; validity window end |
| `nonce` | REQUIRED | string, non-empty | Carried for future replay protection (§8) |
| `kid` | REQUIRED | string, non-empty | Signing key identifier |
| `sig` | REQUIRED | string, non-empty | Detached signature over the Canonical Form (§4) |
...
**AT-SCHEMA-004.** `expires_at` MUST be strictly greater than `not_before`.
A document violating this ordering is malformed and MUST be rejected during
structural verification (AT-VERIFY-002), not merely at time-validity
checking (AT-VERIFY-008).
...
# 4. Canonical Form and Signature (AT-SIG)
...
# 5. Attester Trust Model (AT-TRUST)
...
- an `evidence_type` not present in the resolved anchor's permitted set
  (this specific failure is reported as a distinct verification step, §7).
...
# 6. Verification Order (AT-VERIFY)
...
type, or violates AT-SCHEMA-004's timestamp ordering MUST resolve to
`INVALID`.
...
verified attestation (see §9, AT-AUTH-002).
...
corresponding expected value; when the caller supplies none, the check is
vacuously satisfied and MUST NOT itself cause `INVALID`.
...
expected bindings (§9, AT-AUTH-001). It is never a statement that `claims`
is factually or objectively correct.

---

# 7. Evidence-Type Authorization (AT-TYPE)
...
# 9. Non-Authority Boundary (AT-AUTH)
...
integrity-protected, and correctly bound."

**AT-AUTH-002.** The EvidenceAttestation schema (§3) and the
`AttestationVerificationResult` structure (§6) MUST NOT contain a field
...
# 10. Relationship to Existing MCC-Core Artifacts (AT-REL)
...
scope if it satisfies AT-SCHEMA-001 through AT-SCHEMA-005, AT-SIG-001
through AT-SIG-005, AT-TRUST-001 through AT-TRUST-005, AT-VERIFY-001
through AT-VERIFY-010, AT-TYPE-001, AT-REPLAY-001 through AT-REPLAY-003,
AT-AUTH-001 through AT-AUTH-003, and AT-REL-001 through AT-REL-003.

**AT-CONF-002.** `src/mcc_attestation/` is the reference implementation of
this Draft specification as of PR-1. See
...
- Nonce/replay consumption against a registry (§8).
