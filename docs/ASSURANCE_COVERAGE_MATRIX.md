...
## Summary counts (at the point this matrix was written — 71B)

- **IMPLEMENTED:** 43 line items — every workstream has real, passing, black-box tests against a live 3-process deployment. This now includes genuine mandate-forgery containment (C9-C18), `min_`/`allowed_` constraint types (C20-C23, see `assurance/tests/test_mandate_containment.py`), a durable Hypothesis seed-persistence corpus (H), a real kernel-enforced network-segmentation proof (A, N1-N3, `assurance/sut/network_segmentation.py`, 71A), and — landing in this PR — real network-partition fault injection distinct from process-kill (`test_e3b_network_partition_from_shared_backend_fails_closed_then_recovers`), moved out of BLOCKED BY ENVIRONMENT below.
- **PARTIALLY IMPLEMENTED:** 4 line items (D's corpus exhaustiveness, K's 4/5 adapters locally, L's exact CLI interface + unexercised external mode, CI's required-status-check).
- **NOT IMPLEMENTED:** 4 line items (`EXECUTION_UNKNOWN` external observability, signed external audit checkpoints, PlusCal, external-mode live exercise against a genuinely remote deployment).
- **BLOCKED BY ENVIRONMENT:** 3 line items (multi-node/multi-host E, Redis Cluster/Sentinel failover, production-scale I model checking) — all explicitly infrastructure-gated, not skipped by choice. Both network-partition (this PR) and network-segmentation (A, 71A) are genuinely IMPLEMENTED despite the same environment constraints, using host-local iptables and Linux network namespaces respectively instead of Docker — moved out of this category.
