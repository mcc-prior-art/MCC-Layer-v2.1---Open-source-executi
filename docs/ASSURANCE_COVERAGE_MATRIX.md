...
| Requirement | Status | PR | Evidence |
|---|---|---|---|
| Mutation testing of specific security-critical defects | **IMPLEMENTED** (26: the original 13, plus a 13-defect extension covering every `GateResult(False, ...)` fail-open site in `src/mcc_core/gate.py` — see below) | 71C | `mutation/defects.py` |
| 100% detection requirement | **IMPLEMENTED** (26/26; the original 13 required real investigation and 2 rounds of fixes — see `mutation/detectors.py`'s own docstring for what was wrong and why. The 13-defect extension closed a SECOND, later-found gap — see next row) | 71C | `python -m mutation` exit 0 |
| Exhaustive fail-open coverage of the execution gate itself | **IMPLEMENTED** — a pre-merge direct-verification addendum identified all 14 `GateResult(False, ...)` call sites in `ExecutionGate._verify()`/`verify()` (the only mutant *shape*, out of the generic sweep below, that is an actual authorization bypass) and applied each directly to the real `gate.py`, one at a time, against a real 7-file oracle (`tests/test_mcc_core.py`, `tests/test_gateway.py`, `tests/test_coordinator.py`, `tests/test_mandate.py`, `tests/test_mandate_http.py`, `assurance/tests/test_exclusive_execution_path.py`, `assurance/tests/test_decision_authority_containment.py`). Result: 12/14 caught, 2 survived (the `verify()` exception handler; the `nbf`/`exp` type-check). Both gaps were closed with new regression tests (`tests/test_mcc_core.py::test_gate_denies_when_verify_raises_unexpectedly`, `tests/test_mcc_core.py::test_gate_denies_malformed_time_window`) and all 14 sites were made permanent, individually-named, CI-gated defects (`mutation/defects.py`'s `gate-*-fail-open` block) — 14/14 now detected via the isolated-repo-copy harness, not a one-off script | 71C | `mutation/defects.py`; `python -m mutation` exit 0; `docs/ASSUMPTIONS_AND_LIMITS.md` |
| Generic/exhaustive mutation operator sweep (e.g. via `mutmut`/`cosmic-ray`) across the wider codebase | **PARTIALLY IMPLEMENTED** — `mutmut` (installed, `[tool.mutmut]` in `pyproject.toml`) run against `src/mcc_core/gate.py` (the single most security-critical file): 225 generic AST-level mutants generated, 97 killed / 128 survived against a NARROW, fast oracle (`tests/test_coordinator.py` alone), 116 killed / 109 survived against a broadened 9-file oracle. Of these, the only behaviorally-meaningful shape (a `False`→`True` fail-open flip) is now covered exhaustively and permanently by the row above; the remaining generic survivors are dominated by behaviorally-inert mutations (`allowed: False→None`, arg-drop mutants caught by the gate's own exception handler, cosmetic string recasing, unasserted constant tweaks) and were not individually re-triaged one-by-one, since none represent an unaudited fail-open shape. This is genuine tool output, not fabricated, but the wider-codebase sweep the task named remains only narrowly exercised, not comprehensively triaged | 71C | `pyproject.toml`'s `[tool.mutmut]`; `docs/ASSUMPTIONS_AND_LIMITS.md` |
...
## Summary counts (at the point this matrix was written — 71C)

- **IMPLEMENTED:** 44 line items — every workstream has real, passing, black-box tests against a live 3-process deployment. This includes genuine mandate-forgery containment (C9-C18), `min_`/`allowed_` constraint types (C20-C23, see `assurance/tests/test_mandate_containment.py`), a durable Hypothesis seed-persistence corpus (H, 71A), a real kernel-enforced network-segmentation proof (A, N1-N3, `assurance/sut/network_segmentation.py`, 71A), real network-partition fault injection distinct from process-kill (`test_e3b_network_partition_from_shared_backend_fails_closed_then_recovers`, 71B), and TLA+ model checking (Workstream I, this PR).
- **PARTIALLY IMPLEMENTED:** 4 line items (D's corpus exhaustiveness, J's generic mutation sweep (this PR — scoped to one file, narrow oracle, one flagged survivor spot-checked against the full suite and found caught; see the Workstream J row for the honest detail, not counted as full IMPLEMENTED), K's 4/5 adapters locally, CI's required-status-check).
- **NOT IMPLEMENTED:** 4 line items as of 71C specifically (`EXECUTION_UNKNOWN` external observability, signed external audit checkpoints, PlusCal, external-mode live exercise against a genuinely remote deployment — the last two close later, on 71D).
- **BLOCKED BY ENVIRONMENT:** 3 line items (multi-node/multi-host E, Redis Cluster/Sentinel failover, production-scale I model checking) — all explicitly infrastructure-gated, not skipped by choice. Both network-partition (71B) and network-segmentation (A, 71A) are genuinely IMPLEMENTED despite the same environment constraints, using host-local iptables and Linux network namespaces respectively instead of Docker — moved out of this category.
- **NOT VERIFIED IN CI (as of this branch):** the CI workflow's own real-run status and the 4 framework-dependent Workstream K adapters' CI execution are tracked and updated on 71D, downstream of this branch — see that branch's own copy of this matrix for the current, verified state.
