...
- **IMPLEMENTED:** 46 line items — every workstream has real, passing, black-box tests against a live 3-process deployment. This includes genuine mandate-forgery containment (C9-C18), `min_`/`allowed_` constraint types (C20-C23), a durable Hypothesis seed-persistence corpus (H, 71A), a real kernel-enforced network-segmentation proof (A, N1-N3, 71A), real network-partition fault injection (71B), TLA+ model checking (Workstream I, 71C), and — landing in this PR — the literal `mcc-assurance --target/--actuator` CLI interface (L).
- **PARTIALLY IMPLEMENTED:** 5 line items (D's corpus exhaustiveness, J's generic mutation sweep — scoped to one file with a narrow oracle, see that row for the honest detail, K's 4/5 adapters locally, external mode's still-untested genuinely-remote/cross-organization-trust-boundary claim specifically — the CLI mechanism and code path ARE now implemented and genuinely exercised (L1-L4), see above — CI's required-status-check).
- **NOT IMPLEMENTED:** 4 line items (`EXECUTION_UNKNOWN` external observability — confirmed not a normative gap, see that row — signed external audit checkpoints, constraint types beyond one on the actuator's OWN HTTP-egress config specifically (the general mechanism IS covered via the mandate path), PlusCal — the checkpoints item closes later in this same PR).
- **BLOCKED BY ENVIRONMENT:** 4 line items (multi-node/multi-host E, Redis Cluster/Sentinel failover, production-scale I model checking, the 4 framework-dependent Workstream K adapters locally — the last of these is later reclassified to IMPLEMENTED once this PR's real CI run confirms it, see below) — all explicitly infrastructure-gated, not skipped by choice.
- **NOT VERIFIED IN CI (as of this point in the branch):** the CI workflow's own real-run status is confirmed later in this same PR, once the workflow actually runs against this branch's HEAD — see the CI section's own row for the run URL once available.
