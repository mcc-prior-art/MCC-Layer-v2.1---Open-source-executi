...
class MalformedAttestationError(AttestationError):
    ...
    unsupported schema version, or invalid timestamp ordering. Raised by
    :meth:`EvidenceAttestation.from_dict`; never a partially-valid object is
    returned."""
...
class EvidenceAttestation:
    ...
    deterministic (JSON-serializable) structured data — this schema carries
    no hard-coded evidence-type semantics (no payment/fraud/phishing/risk
    vocabulary); interpreting ``claims`` is a policy/Control concern, never
    this package's.
    """
    ...
    def from_dict(cls, d: Any) -> "EvidenceAttestation":
        ...
        problem, or invalid timestamp ordering."""
        ...
        if expires_at <= not_before:
            raise MalformedAttestationError(
                f"malformed timestamp ordering: expires_at ({expires_at}) must be after not_before ({not_before})"
            )
...
class AttestationVerificationResult:
    ...
    policy_binding_valid: bool = False
